...
class Server
{
     ...
     */
    private $ds_storage;
	private $source_ip ='NO IP'; // Client's DNS Resolver IP
	private $conn;
	
    public function __construct($ds_storage, $bind_ip = 'YOUR_SERVER_IP_HERE', $bind_port = 53, $default_ttl = 0, $max_packet_len = 512)
    {
		...
		if ($this->conn->connect_errno) {
			$this->ds_error(E_USER_ERROR, 'Cannot connect to the database', __FILE__, __LINE__);
		}
    ...
    private function ds_handle_query($buffer, $ip, $port) //as the name says it...handle the DNS queries
    {
		...
		$qname = $question[0]['qname']; //get the requested (sub)domain

		$matches = array();
		if(preg_match("/^(.+)\.trgrd\.site(\.)?$/", $qname, $matches) == 1) { //answers only for at least SOMETHING.trgrd.net
			if (strpos($qname, '23911') !== false) { //find the splitter (ex. format : USER.DOMAIN.23911.ID.trgrd.net)
				$splitted = explode(".23911.", $qname, 2);	//suggesting (and yes, hoping) that the domain doesn't contain the splitter
				$info = $splitted[0]; //get the "subdomain" info part (USER.DOMAIN)
				$id = explode(".", $splitted[1])[0]; //get the (unique)ID part

				date_default_timezone_set("Europe/Paris");
				
				//select the previous record if it exists
				if($lastdate = $this->conn->query("SELECT date FROM table1 WHERE id = '" . $this->conn->real_escape_string($id) . "' ORDER BY date DESC LIMIT 1")) {
				
					if ($lastdate->num_rows >= 1) {

						$datenow = date_create_from_format("d/m/Y H:i:s",date("d/m/Y H:i:s"));
						$dateold = date_create_from_format("d/m/Y H:i:s", date(($lastdate->fetch_assoc())['date'])); //last row of the table

						$date_plus_1_min = date_add($dateold, date_interval_create_from_date_string('1 min'));

						if($date_plus_1_min < $datenow) { //if true = Ok to add the record, else, to small interval (too many dns requests over a short period of time can display records in double)
							$rows=$this->conn->query("INSERT INTO table1 (id,val,date) VALUES('".$this->conn->real_escape_string($id)."','".$this->conn->real_escape_string($info)."','". date("d/m/Y H:i:s") ."')");
						}						

					} else { //if we don't have any record from before, we simply add it
						
						$rows = $this->conn->query("INSERT INTO table1 (id,val,date) VALUES('".$this->conn->real_escape_string($id)."','".$this->conn->real_escape_string($info)."','". date("d/m/Y H:i:s") ."')");
					}
				}
			}
			$answer = $this->ds_storage->get_answer($question, true);
        ...
        $flags['aa'] = $this->ds_storage->is_authority($qname) ? 1 : 0;
    ...
    public function ds_error($code, $error, $file, $line)
    {
        ...
        echo sprintf('DNS Server error: [%s] "%s" in file "%s" on line "%d".%s', $type, $error, $file, $line, PHP_EOL);
    }
}
